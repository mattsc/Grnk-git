return {
    init = function(ai)
        ...
        function sheep:close_enemy_eval()
            ...
            -- OR once the dogs themselves were attacked, they'll go after Grnk's units

            local dogs = wesnoth.get_units { side = wesnoth.current.side, type = 'Dog',
                formula = '$this_unit.moves > 0'
            }
            if (not dogs[1]) then return 0 end

            local S = H.get_variable_array("S")
            if S and S[1] and S[1].dogs_attacked then
                --print("eval: dogs have been attacked")
                return 300000
            end
            ...
            if enemies[1] and sheep[1] then return 300000 end
            return 0
        end
        ...
        function sheep:close_enemy_exec()
            ...
            -- Or, once a dog was attacked, they will blindly go after all units on Grnk's side (Side 1)
            local S = H.get_variable_array("S")
            if S and S[1] and S[1].dogs_attacked then
                enemies = wesnoth.get_units{side = 1}
                --print("exec: dogs have been attacked, #enemies", #enemies)
            end
