return {
	...
	function defend_pass:move_eval()
	    ...
	    for i,l in ipairs(attack_locs) do
	        ...
	        local unit_in_way = wesnoth.get_units { x = l[1], y = l[2] }[1]
	        if unit_in_way then
	...
	function defend_pass:move_exec()
	    ...
	    if self.data.lu_target then
	        ...
	        ai.attack(self.data.unit, self.data.lu_target, self.data.lu_weapon-1)
	    end
	    ...
	    for i,a in ipairs(attackers) do
	        ...
	        for j,t in ipairs(targets) do
	            ...
	            for weapon in H.child_range(a.__cfg, "attack") do
	                ...
	                local rating = 0
                        -- We change the attack rating here, making it more aggressive than in AI-Demos
	                -- This is an acceptable attack if:
	                -- 1. Chance to die is 0 and expected damage dealt certain fraction (dependent on target level) more than damage received
                        -- 2. or CTK > 30%
                        -- 3. or if it's Grnk or the side leader
                        damage = a.hitpoints - att_stats.average_hp
                        damage_target = t.hitpoints - def_stats.average_hp
                        print('attacker/defender expected damage: ', a.id, t.id, damage, damage_target, n_weapon, (2. - t.__cfg.level * 0.5))
	                if ((att_stats.hp_chance[0] == 0) and (damage_target >= damage * (2. - t.__cfg.level * 0.5)))
                            or (def_stats.hp_chance[0] > 0.3)
                            or (t.id == 'Grnk') or t.canrecruit
	...
	function defend_pass:attack_exec()
	    ...
	    ai.attack(self.data.attacker, self.data.target, self.data.weapon-1)
